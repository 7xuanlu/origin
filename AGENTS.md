...
| **L4 CI on PR** | Fail-closed differential plan: affected lib, integration, contract, platform, and HTTP smoke owners only; aggregate `conclusion` verifies every expected job. An exact same-repository Release PR whose current-main diff passes the shared semantic release validator omits the duplicate macOS/Windows debug matrix while retaining Linux canonical acceptance and all four shipped-target release preflights. Pushes to `main` retain full workspace/platform coverage plus the Windows release-profile cache warmer. | GitHub (`ci.yml`) | Every PR | target ≤20min | Yes (required) |
