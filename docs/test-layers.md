...
| **L3 pre-push** | Reader-inventory freshness check (Python, no compile); `WENLAN_PUSH_FULL=1` opts into the planner-selected Clippy + lib-test closure | Local | `git push` | <2s (opt-in run: change-dependent) | Yes (fast checks only) |
