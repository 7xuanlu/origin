...
| **L7b first-run gauntlet** | Installs the *published* release artifacts (DMG, `npx wenlan setup`, Homebrew tap, `npx wenlan-mcp`, Linux tarball, Windows zip + NSIS, `cargo install`, Claude Code plugin) on clean hosted runners the way a new user would; every channel reports a `findings.tsv` and the summary job turns the run red on any FAIL. Scripts under `scripts/first-run/` run by hand with the same env. | GitHub (`first-run-gauntlet.yml`) | Manual dispatch, after a release is published | ~30min | No (findings) |
