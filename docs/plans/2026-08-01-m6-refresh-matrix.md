...
|---|---|---|
| `dirty` | `pages.stale_reason IS NOT NULL` | `set_page_stale` (`crates/wenlan-core/src/db.rs:46111`) |
| `queued` | no row of its own — the page is in the sweep's selection | `list_stale_pages` (`crates/wenlan-core/src/db.rs:46347`); the sweep *is* the queue |
...
| `finalized` | `pages` row advanced by the M5 finalizer | `try_update_page_content` (`crates/wenlan-core/src/db.rs:42418`) |
...
| dependency generation | `space_graph_state.grouping_generation` (`crates/wenlan-core/src/db.rs:10470`) | same counter as S0-1 |
...
| 5 | a new evidence link on the page | `link_page_evidence` (`crates/wenlan-core/src/db.rs:45219`), statement at `:45244`-`:45249` | `source_updated` | **no — see finding F1** |
| 6 | the citation-backfill lane's annotate write | `set_page_citations_with_changelog_at_version` (`crates/wenlan-core/src/db.rs:45174`, statement at `:45184`-`:45186`) | — mutates `pages`, marks nothing, advances no version | **no — see finding F2** |
...
| In-statement guard on the UPDATE | `crates/wenlan-core/src/db.rs:42546` and `:42567` (and `:42668`, `:42688` on the second arm) — `AND COALESCE(user_edited, 0) = 0` | durable: a page that became human-owned after the read makes the UPDATE match zero rows, so the write silently becomes a no-op rather than an overwrite |
...
| the human accepts | `try_accept_page_revision` (`crates/wenlan-core/src/db.rs:42378`) applies the content and consumes the card **in one transaction** |
...
**`MemoryDB::try_update_page_content`, `crates/wenlan-core/src/db.rs:42418`.**
...
|---|---|---|
| `try_update_page_content_with_changelog_at_source_revision` | source revision | `crates/wenlan-core/src/db.rs:42227` |
| `try_update_page_content_with_changelog_at_version` | page version | `:42309` |
| `try_update_page_growth_at_versions` | page version + source revision + memory version | `:42344` |
| `try_accept_page_revision` | optional page version, consumes the card | `:42382` (doc `:42379`-`:42381`) |

Its doc comment (`:42409`-`:42416`) states the atomicity contract: the changelog
...
> `crates/wenlan-core/src/db.rs:42480`.
...
(`crates/wenlan-core/src/db.rs:45219`) runs, when and only when the evidence row
was actually new (`if inserted > 0`, `:45242`):

```sql
-- crates/wenlan-core/src/db.rs:45244-45249
UPDATE pages
   SET citations = NULL,
...
`crates/wenlan-core/src/db.rs:42227`), while the function's own doc comment
(`:45215`-`:45218`) states that *"the content generation stays untouched"* —
...
(`crates/wenlan-core/src/db.rs:45174`) issues:

```sql
-- crates/wenlan-core/src/db.rs:45184-45186
...
`try_update_page_content`'s documented rule (`:42411`-`:42414`) resets
...
eligible for annotation"* (`:42414`) — and the annotate statement's
`AND citations IS NULL` guard (`:45186`) is what makes re-annotation the natural
...
IMMEDIATE`** (`crates/wenlan-core/src/db.rs:42480`, plain `conn.execute("BEGIN",
