...
- **`GET /api/memory/recent` → `yes`.** `handle_recent_memories`
  (`server/memory_routes.rs:2845`) → `list_recent_memories_scoped`
...
- **`GET /api/memory/unconfirmed` → `yes`.**
  `handle_list_unconfirmed_memories` (`server/memory_routes.rs:2868`) →
...
- **`GET /api/briefing` → `yes`.** `handle_get_briefing`
  (`server/memory_routes.rs:2110`) → `generate_briefing_scoped`
...
- **`GET /api/memory/{id}/revisions` → `yes`.** `handle_get_memory_revisions`
  (`server/memory_routes.rs:4474`) → `walk_supersede_chain_scoped`
...
- **`GET /api/memory/pinned` → `yes`.** `handle_list_pinned_memories`
  (`server/memory_routes.rs:2196`) → `list_pinned_memories_scoped`
...
| `server/memory_routes.rs:2549` | `handle_update_page` | `pub` | `get_page` |
| `server/memory_routes.rs:2624` | `handle_refresh_page` | `pub` | `get_page` |
