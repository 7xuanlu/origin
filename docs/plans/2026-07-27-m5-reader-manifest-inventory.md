...
- **`GET /api/memory/recent` → `yes`.** `handle_recent_memories`
  (`server/memory_routes.rs:2961`) → `list_recent_memories_scoped`
...
- **`GET /api/memory/unconfirmed` → `yes`.**
  `handle_list_unconfirmed_memories` (`server/memory_routes.rs:2984`) →
...
- **`GET /api/briefing` → `yes`.** `handle_get_briefing`
  (`server/memory_routes.rs:2226`) → `generate_briefing_scoped`
...
- **`GET /api/memory/{id}/revisions` → `yes`.** `handle_get_memory_revisions`
  (`server/memory_routes.rs:4590`) → `walk_supersede_chain_scoped`
...
- **`GET /api/memory/{id}/versions` → `yes`.** `handle_get_version_chain`
  (`server/memory_routes.rs:1865`) → `get_version_chain_scoped`
...
- **`GET /api/memory/by-ids` → `yes`.** `handle_get_memories_by_ids`
  (`server/memory_routes.rs:1836`) → `get_memories_by_source_ids_scoped`
...
- **`GET /api/memory/{id}/detail` → `yes`.** `handle_get_memory_detail`
  (`server/memory_routes.rs:1811`) → `get_memory_detail_scoped`
...
- **`GET /api/memory/pinned` → `yes`.** `handle_list_pinned_memories`
  (`server/memory_routes.rs:2312`) → `list_pinned_memories_scoped`
...
| `server/memory_routes.rs:2665` | `handle_update_page` | `pub` | `get_page` |
| `server/memory_routes.rs:2740` | `handle_refresh_page` | `pub` | `get_page` |
