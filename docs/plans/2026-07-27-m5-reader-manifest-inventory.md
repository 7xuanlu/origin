...
handler-movement slice is 92 in `router.rs`, 13 in `routes.rs`, 2 in
...
`page_map_routes.rs`, 6 in `profile_agents_routes.rs`, 1 in `websocket.rs`, 5
in `repair_routes.rs`, and 2 in `lint_routes.rs`.
...
- **`GET /api/memory/rejections`.** `handle_get_rejections`
  (`memory_routes.rs:1744`) calls `get_rejections` (`db.rs:41112`), which
...
- **`GET /api/activities` → `yes`.** `agent_activity.detail` carries page
  ...
  `handle_list_activities` (`server/memory_routes.rs:2478`). The original
...
- **`GET /api/memory/nurture` → `yes`.** `handle_get_nurture_cards`
  (`server/memory_routes.rs:1721`) → `get_nurture_cards_scoped`
...
- **`GET /api/memory/recent` → `yes`.** `handle_recent_memories`
  (`server/memory_routes.rs:3821`) → `list_recent_memories_scoped`
...
- **`GET /api/memory/unconfirmed` → `yes`.**
  `handle_list_unconfirmed_memories` (`server/memory_routes.rs:3844`) →
...
- **`GET /api/indexed-files` → `yes`.** `handle_list_indexed_files`
  (`server/memory_routes.rs:2186`) → `list_indexed_files_scoped`
...
- **`GET /api/briefing` → `yes`.** `handle_get_briefing`
  (`server/memory_routes.rs:3086`) → `generate_briefing_scoped`
...
- **`GET /api/memory/{id}/revisions` → `yes`.** `handle_get_memory_revisions`
  (`server/memory_routes.rs:5450`) → `walk_supersede_chain_scoped`
...
- **`GET /api/memory/{id}/versions` → `yes`.** `handle_get_version_chain`
  (`server/memory_routes.rs:2725`) → `get_version_chain_scoped`
...
- **`POST /api/memory/list` → `yes`.** `handle_list_memories`
  (`server/memory_routes.rs:1045`) → `list_filtered_confirmed_scoped`
  (`db.rs:29220`), which pushes `pending_revision = 0` at `db.rs:29274`. The
  handler forwards `req.confirmed` unchanged (`server/memory_routes.rs:1062`),
...
- **`GET /api/memory/by-ids` → `yes`.** `handle_get_memories_by_ids`
  (`server/memory_routes.rs:2696`) → `get_memories_by_source_ids_scoped`
...
- **`GET /api/memory/{id}/detail` → `yes`.** `handle_get_memory_detail`
  (`server/memory_routes.rs:2671`) → `get_memory_detail_scoped`
...
- **`GET /api/memory/pinned` → `yes`.** `handle_list_pinned_memories`
  (`server/memory_routes.rs:3172`) → `list_pinned_memories_scoped`
...
- **`GET /api/home-stats` → `yes`, and its gate is not the one the tier name
  suggests.** `handle_get_home_stats` (`server/memory_routes.rs:1326`) →
...
| `core/db/scoped_entities.rs:12` | `list_entities_scoped` | `pub` | **exposure** — `server/memory_routes.rs:1247` |
| `core/db/scoped_entities.rs:84` | `get_entity_detail_scoped` | `pub` | **exposure** — `server/memory_routes.rs:1263` |
| `core/db/scoped_entities.rs:291` | `list_recent_relations_scoped` | `pub` | **exposure** — `server/knowledge_routes.rs:78` |
| `core/db/scoped_entities.rs:615` | `search_entities_by_vector_scoped` | `pub` | **exposure** — `server/memory_routes.rs:1298` |
...
| `server/memory_routes.rs:1234` | `handle_list_entities` | `pub` | `list_entities_scoped` |
| `server/memory_routes.rs:1253` | `handle_get_entity_detail` | `pub` | `get_entity_detail_scoped` |
| `server/memory_routes.rs:1285` | `handle_search_entities` | `pub` | `search_entities_by_vector_scoped` |
...
| `server/memory_routes.rs:1975` | `handle_create_page` | `pub` | `get_page` |
| `server/memory_routes.rs:3525` | `handle_update_page` | `pub` | `get_page` |
| `server/memory_routes.rs:3600` | `handle_refresh_page` | `pub` | `get_page` |
