...
three handler-movement slices is 65 in `router.rs`, 13 in `routes.rs`, 2 in
...
`page_map_routes.rs`, 14 in `entity_graph_routes.rs`, 13 in `spaces_routes.rs`, 6 in
...
- **`GET /api/memory/rejections`.** `handle_get_rejections`
  (`memory_routes.rs:1355`) calls `get_rejections` (`db.rs:41112`), which
...
- **`GET /api/activities` → `yes`.** `agent_activity.detail` carries page
  ...
  `handle_list_activities` (`server/memory_routes.rs:1897`). The original
...
- **`GET /api/memory/nurture` → `yes`.** `handle_get_nurture_cards`
  (`server/memory_routes.rs:1332`) → `get_nurture_cards_scoped`
...
- **`GET /api/memory/recent` → `yes`.** `handle_recent_memories`
  (`server/memory_routes.rs:3240`) → `list_recent_memories_scoped`
...
- **`GET /api/memory/unconfirmed` → `yes`.**
  `handle_list_unconfirmed_memories` (`server/memory_routes.rs:3263`) →
...
- **`GET /api/indexed-files` → `yes`.** `handle_list_indexed_files`
  (`server/memory_routes.rs:1797`) → `list_indexed_files_scoped`
...
- **`GET /api/briefing` → `yes`.** `handle_get_briefing`
  (`server/memory_routes.rs:2505`) → `generate_briefing_scoped`
...
- **`GET /api/memory/{id}/revisions` → `yes`.** `handle_get_memory_revisions`
  (`server/memory_routes.rs:4869`) → `walk_supersede_chain_scoped`
...
- **`GET /api/memory/{id}/versions` → `yes`.** `handle_get_version_chain`
  (`server/memory_routes.rs:2144`) → `get_version_chain_scoped`
...
- **`POST /api/memory/list` → `yes`.** `handle_list_memories`
  (`server/memory_routes.rs:1010`) → `list_filtered_confirmed_scoped`
  (`db.rs:29220`), which pushes `pending_revision = 0` at `db.rs:29274`. The
  handler forwards `req.confirmed` unchanged (`server/memory_routes.rs:1027`),
...
- **`GET /api/memory/by-ids` → `yes`.** `handle_get_memories_by_ids`
  (`server/memory_routes.rs:2115`) → `get_memories_by_source_ids_scoped`
...
- **`GET /api/memory/{id}/detail` → `yes`.** `handle_get_memory_detail`
  (`server/memory_routes.rs:2090`) → `get_memory_detail_scoped`
...
- **`GET /api/memory/pinned` → `yes`.** `handle_list_pinned_memories`
  (`server/memory_routes.rs:2591`) → `list_pinned_memories_scoped`
...
- **`GET /api/home-stats` → `yes`, and its gate is not the one the tier name
  suggests.** `handle_get_home_stats` (`server/memory_routes.rs:1104`) →
...
| `server/memory_routes.rs:2944` | `handle_update_page` | `pub` | `get_page` |
| `server/memory_routes.rs:3019` | `handle_refresh_page` | `pub` | `get_page` |
...
2. asserts the derived set **exactly equals** this file's rows — extra and
   ...
   (`router.rs:338`). An arithmetic that reads only the two
