...
- **`GET /api/activities` → `yes`.** `agent_activity.detail` carries page
  ...
  `handle_list_activities` (`server/memory_routes.rs:1797`). The original
...
- **`GET /api/memory/recent` → `yes`.** `handle_recent_memories`
  (`server/memory_routes.rs:3140`) → `list_recent_memories_scoped`
...
- **`GET /api/memory/unconfirmed` → `yes`.**
  `handle_list_unconfirmed_memories` (`server/memory_routes.rs:3163`) →
...
- **`GET /api/briefing` → `yes`.** `handle_get_briefing`
  (`server/memory_routes.rs:2405`) → `generate_briefing_scoped`
...
- **`GET /api/memory/{id}/revisions` → `yes`.** `handle_get_memory_revisions`
  (`server/memory_routes.rs:4769`) → `walk_supersede_chain_scoped`
...
- **`GET /api/memory/{id}/versions` → `yes`.** `handle_get_version_chain`
  (`server/memory_routes.rs:2044`) → `get_version_chain_scoped`
...
- **`GET /api/memory/by-ids` → `yes`.** `handle_get_memories_by_ids`
  (`server/memory_routes.rs:2015`) → `get_memories_by_source_ids_scoped`
...
- **`GET /api/memory/{id}/detail` → `yes`.** `handle_get_memory_detail`
  (`server/memory_routes.rs:1990`) → `get_memory_detail_scoped`
...
- **`GET /api/memory/pinned` → `yes`.** `handle_list_pinned_memories`
  (`server/memory_routes.rs:2491`) → `list_pinned_memories_scoped`
...
| `server/memory_routes.rs:2844` | `handle_update_page` | `pub` | `get_page` |
| `server/memory_routes.rs:2919` | `handle_refresh_page` | `pub` | `get_page` |
