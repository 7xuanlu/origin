...
| B6 | `reserved_exclusive` → `released` | recovery scan (S0-5) | no live lease for the owning candidate | `released_at = unixepoch()`, `reason='lease_lost'`; group returns to `waiting_frontier` (**F5**), same transaction | The crash-recovery edge. Without it, a killed process's reservation would hide its group from the frontier forever (S0-5's rationale). |
