...
- **Cadence:** L6 (push to `main`). `ranking_drift_vs_golden` is `#[ignore]`d and picked up by the dedicated `main-canary.yml` command `cargo nextest run -p wenlan-core --lib --run-ignored=only eval::retrieval` via substring match. The metric + regression-injection tests are synthetic (no model) and run per-PR. NOT a pre-push gate (wenlan has none by design).
