# Rust eval runner reference

Detailed runner shapes, cache behavior, experiment methodology, statistics, and
historical receipts. Read the active `AGENTS.md` first and load this file only for an
eval task that needs the corresponding detail.

Read alongside the active root/core instructions and `app/eval/REFERENCE.md` when the
task needs fixture or artifact detail.
...
- `is_single_run`: always `true` for single-run tests. Any baseline with `is_single_run = true` must not be cited externally (see `app/eval/REFERENCE.md` under eval citation discipline).
...
3. **Faithful per-question isolation only.** The consolidated/merged cross-conversation substrate (one shared 11397-memory haystack) is BANNED for CE pool / on-off decisions — a question retrieves another's evidence, inflating deep-pool gains. Code teeth: `run_fullpipeline_lme` refuses `DbSource::Consolidated` for confounder-isolation arms unless `ALLOW_CONSOLIDATED_CE=1`. See root `AGENTS.md` + memory `feedback_no_consolidated_ce_substrate`.
   ...
   - **PROVENANCE WARNING:** the historical CE figures (`+0.178 NDCG@10` — retained under the `WENLAN_RERANK_SKIP_PREFERENCE` flag in `crates/wenlan-core/REFERENCE.md`; `+0.233@pool=50`, no longer in active instructions) were measured on **oracle (pool-blind)** or **consolidated (banned)** substrates — NOT faithful deep-S. Do NOT cite them for a deep-S pool/CE decision; regenerate retrieval NDCG on `SeedPerQuestion + _s`.
...
Results of the sweeps run under this methodology — the deep-S N=31 pool/model sweep and its substrate caveats — are in [`docs/eval-sweep-results.md`](../../../../docs/eval-sweep-results.md).
...
- **Cadence:** L6 (relevant core/eval-owner push to `main`, plus manual dispatch). `ranking_drift_vs_golden` is `#[ignore]`d and selected by its exact full test name alongside `test_run_quality_cost_eval_basic`; never use the broad `eval::retrieval` substring because it also selects manual/scaling benches. The metric + regression-injection tests are synthetic (no model) and run per-PR. NOT a pre-push gate (wenlan has none by design).
...
- Fixture population, env vars, seed scripts, baseline layout: see `app/eval/REFERENCE.md`.
- Citation rules (single-run, schema-version, receipt-only): see `app/eval/REFERENCE.md` under eval citation discipline.
