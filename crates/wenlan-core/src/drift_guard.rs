...
fn doc_path_references_resolve() {
    ...
    for f in git_ls_files(&root, "*.md") {
        ...
        // `docs/contracts/` is skipped for a different reason than the two
        // gitignored scratch dirs: those files ARE tracked and load-bearing, but
        // their prose cross-references point into the untracked design history
        // they were promoted out of. Their real integrity is enforced by the
        // tests that parse them (`truth_manifest_test`, `m6::catalog_test`,
        // `scripts/m5-reader-sweep.py --check`), not by this path resolver.
        if f.starts_with("docs/plans/")
            || f.starts_with("docs/superpowers/")
            || f.starts_with("docs/contracts/")
