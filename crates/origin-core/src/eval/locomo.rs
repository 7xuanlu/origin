...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocomoRetrievedQuestion {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocomoAnsweredQuestion {
    pub conversation_id: String,
    pub question: String,
    pub ground_truth: String,
    pub category: u8,
...
#[derive(Debug, Clone, Serialize)]
pub struct LocomoAccuracyResult {
    pub conversation_id: String,
    pub question: String,
    pub ground_truth: String,
    pub category: u8,
...
#[derive(Debug, Clone, Serialize)]
pub struct LocomoCategoryAccuracy {
    pub category: u8,
    pub category_name: String,
    pub total: usize,
    pub correct: usize,
...
#[derive(Debug, Clone, Serialize)]
pub struct LocomoAccuracyReport {
    ...
    pub per_category: Vec<LocomoCategoryAccuracy>,
    pub per_question: Vec<LocomoAccuracyResult>,
}

impl LocomoAccuracyReport {
    pub fn to_terminal(&self) -> String {
        let mut out = String::new();
        out.push_str("LoCoMo Canonical Accuracy\n");
        out.push_str("=========================\n");
...
pub fn save_locomo_retrieved(
    questions: &[LocomoRetrievedQuestion],
    path: &std::path::Path,
) -> Result<(), std::io::Error> {
...
pub fn load_locomo_retrieved(
    path: &std::path::Path,
) -> Result<Vec<LocomoRetrievedQuestion>, std::io::Error> {
    let content = std::fs::read_to_string(path)?;
    serde_json::from_str(&content).map_err(std::io::Error::other)
}

pub fn save_locomo_answered(
    questions: &[LocomoAnsweredQuestion],
    path: &std::path::Path,
) -> Result<(), std::io::Error> {
    let json = serde_json::to_string_pretty(questions).map_err(std::io::Error::other)?;
    std::fs::write(path, json)
...
pub fn load_locomo_answered(
    path: &std::path::Path,
) -> Result<Vec<LocomoAnsweredQuestion>, std::io::Error> {
    let content = std::fs::read_to_string(path)?;
    serde_json::from_str(&content).map_err(std::io::Error::other)
}

/// Phase 1: Retrieve context for each LoCoMo QA pair using Origin's hybrid search.
/// Seeds one DB per conversation, then queries per QA pair (skips category 5).
pub async fn retrieve_for_locomo_accuracy(
    path: &std::path::Path,
    search_top_k: usize,
    max_questions_per_conv: usize,
) -> Result<Vec<LocomoRetrievedQuestion>, crate::error::OriginError> {
    let samples = load_locomo(path)?;
    ...
    for (conv_idx, sample) in samples.iter().enumerate() {
        ...
        // Seed DB once per conversation
        let tmp = tempfile::tempdir()
            .map_err(|e| crate::error::OriginError::Generic(format!("tempdir: {e}")))?;
        ...
        for qa in &sample.qa {
            ...
            if q_count >= max_questions_per_conv {
                break;
            }
            let gt = qa
                .answer
            ...
            let context: String = results
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let ctx_tok = crate::eval::token_efficiency::count_tokens(&context);

            retrieved.push(LocomoRetrievedQuestion {
                conversation_id: sample.sample_id.clone(),
...
pub async fn generate_locomo_answers(
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| crate::error::OriginError::Generic(format!("reqwest: {e}")))?;

    // Resume key: (conversation_id, question) tuple
    ...
    let todo: Vec<&LocomoRetrievedQuestion> = retrieved
        ...
        .collect();
    eprintln!(
        "[locomo_answer] {} already answered, {} remaining",
        existing.len(),
        todo.len()
    );
    if todo.is_empty() {
    ...
    for (seq, rq) in todo.into_iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            ...
            if seq % 50 == 0 {
                eprintln!("[locomo_answer] {}/{}...", seq + 1, total);
            }

            let prompt = if rq.context.is_empty() {
                format!(
            ...
            let sys = "Answer the question based on the provided context. Be specific and concise. Respond in 1-3 sentences.";

            match crate::eval::longmemeval::call_anthropic_api(
                &client,
                &api_key,
                &model,
                &prompt,
            ...
            .await
            {
                Ok(answer) => Some(LocomoAnsweredQuestion {
                ...
                Err(e) => {
                    eprintln!("[locomo_answer] failed: {}", e);
                    None
                }
            }
        });
    ...
    eprintln!("[locomo_answer] Total: {} answers", all.len());
    Ok(all)
}

/// Standard LoCoMo judge prompt (matches Mem0 harness / LongMemEval SSU/SSA/MS template).
fn locomo_judge_prompt(question: &str, ground_truth: &str, model_answer: &str) -> String {
    format!(
...
/// Phase 3: Score LoCoMo answers using the Anthropic API as judge.
pub async fn score_locomo_answers(
    answered: &[LocomoAnsweredQuestion],
    judge_model: &str,
    concurrency: usize,
) -> Result<LocomoAccuracyReport, crate::error::OriginError> {
    use std::sync::Arc;
    use tokio::sync::Semaphore;
    ...
    for aq in answered {
        ...
        let handle = tokio::spawn(async move {
            let _permit = sem.acquire().await.unwrap();
            let judge_prompt =
                locomo_judge_prompt(&aq.question, &aq.ground_truth, &aq.model_answer);
            let (judge_response, correct) = match crate::eval::longmemeval::call_anthropic_api(
                &client,
                &api_key,
                &model,
                &judge_prompt,
            ...
            .await
            {
                ...
                Err(e) => {
                    eprintln!("[locomo_judge] error: {}", e);
                    (format!("ERROR: {e}"), None)
                }
            };
            LocomoAccuracyResult {
                conversation_id: aq.conversation_id,
    ...
    if errors > 0 {
        eprintln!("[locomo_aggregate] {} judge errors excluded", errors);
    }
