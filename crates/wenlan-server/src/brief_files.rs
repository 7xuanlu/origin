...
pub async fn import_legacy_status_files(
    ...
    for path in paths {
        ...
        if content
            .starts_with("<!-- Generated by Wenlan from daemon-authoritative Brief state. -->")
        {
...
pub fn render_brief_receipt(brief: &Brief, generated_at: i64) -> String {
    ...
    let mut output = format!(
        "<!-- Generated by Wenlan from daemon-authoritative Brief state. -->\n\
         <!-- Wenlan Brief space-id={}; version={} -->\n\
         <!-- This file is a receipt for inspection, not an input. -->\n\
         # {} — Brief\n\n\
         Generated: {}\n\n\
         ## Last session ({})\n",
        brief.space_id, brief.version, brief.space, generated, last_session_date
    );
...
fn projected_brief_metadata(content: &str) -> Option<(&str, i64)> {
    let line = content
        .lines()
        .find(|line| line.starts_with("<!-- Wenlan Brief space-id="))?;
    let payload = line
        .strip_prefix("<!-- Wenlan Brief space-id=")?
        .strip_suffix(" -->")?;
    let (space_id, version) = payload.split_once("; version=")?;
    Some((space_id, version.parse().ok()?))
}
...
pub fn project_brief_receipt(
    ...
    if let Ok(existing) = std::fs::read_to_string(&destination) {
        if let Some((space_id, version)) = projected_brief_metadata(&existing) {
            if space_id == brief.space_id && version > brief.version {
