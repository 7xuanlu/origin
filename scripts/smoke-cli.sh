...
# old `command -v lsof || fail` gate was protecting, and losing it in
# translation is how a port-conflict or a leaked daemon ships green.
#
# Windows notes, all additive — POSIX behaviour is unchanged:
#   * `mktemp -d` gets an explicit template on every path, and the resulting
#     MSYS path is converted with `native_path` at the DAEMON boundary only:
#     the shell's own `rm -rf` still needs the MSYS spelling. Two variables,
#     never one rewritten variable.
#   * `$!` is an MSYS job pid; `kill -9` on it reaches the MSYS wrapper, not
#     necessarily the native daemon. Both pids are kept (S3) and the stop goes
#     through the identity-checked `force_terminate_process`, never pid alone.
set -euo pipefail
...
# failure fails the script. The PASS line moved in here for that reason — it
# used to print before the EXIT trap ran, so a leaked daemon could not fail the
# run. PASS must not outrank a leak.
#
# Final status = the body's status if non-zero, else cleanup's status. A
# cleanup that exits with its own status alone would turn a failed body into a
# pass whenever the teardown happened to work.
cleanup() {
    local body=$?
    trap - EXIT
    set +e
...
# The old gate sat after `cargo build`, so a checkout that could not run the
# smoke at all paid a full compile before being told so.
...
# Explicit template: macOS mktemp ignores TMPDIR without one, the sandboxed dev
# loop can only write under TMPDIR, and TMPDIR is unset in Git Bash.
DATA_DIR="$(mktemp -d "${TMPDIR:-/tmp}/smoke-cli.XXXXXX")" || fail "mktemp failed"
...
# WENLAN_DATA_DIR, so isolating the data dir alone still writes captures into
# the developer's real pages directory. Written atomically BEFORE the spawn:
# startup reads configuration before it serves HTTP, so a config written after
# the spawn would read back green while startup had already touched the default
# directory.
...
if (( HOST_IS_WINDOWS == 1 )); then
    # `$!` is the MSYS job pid; every Windows process and listener table reports
    # the Windows one. Both are kept: bash can only `wait` on its own child.
    resolve_rc=0
...
# Health alone only proves that something answers on the port.
probe_listener_port "$PORT"
case "$LISTENER_PROBE_STATE" in
    found) ;;
...
# Windows pid on Git Bash, and `$!` itself on POSIX, where the subshell execs
# the daemon and so keeps its own pid.
if (( HOST_IS_WINDOWS == 1 )); then
...
if (( HOST_IS_WINDOWS == 1 )); then
    ...
    # `ps -o command=` reports the command line, so the recorded path is either
    # the whole of it or its first word — the same comparison dev-runtime.sh
    # already ships on POSIX.
...
# One process, no pipe: `sed … | head -1` CAN SIGPIPE sed, and under pipefail
# that is indistinguishable from a parse failure.
...
# step omitted. The expectation is DERIVED from what this invocation asks the
# driver to do — cli-roundtrip.sh makes four unconditional check/check_output
# calls (cli-status, cli-capture, cli-memories, cli-search) and emits no
# conditional row — rather than pasted from an observed run. The ledger is fresh
...
[ -s "$TSV" ] || fail "CLI round-trip recorded nothing"
...
if [ "$actual_rows" != "$want_rows" ]; then
    ...
    fail "CLI round-trip ledger is not the expected (name,status) multiset"
fi
