...
# Reset opt-out flag if present:
test -f ~/Library/Application\ Support/origin/config.json && \
  jq 'del(.auto_start_disabled)' ~/Library/Application\ Support/origin/config.json > /tmp/origin-cfg.json && \
  mv /tmp/origin-cfg.json ~/Library/Application\ Support/origin/config.json
...
| 4 | Tray menu → "Quit Origin" | Tray gone within 2s; daemon dead (`lsof -ti :7878` empty); plists still in `~/Library/LaunchAgents/` (loaded but not running) |
...
| 7 | Open Origin.app from /Applications (after Step 6) | App runs; setup() detects `auto_start_disabled=true`, skips silent install; daemon spawned as Tauri child (fallback mode) |
| 8 | Settings → toggle "Run at login" on | Daemon up; tray active; plists re-installed; `auto_start_disabled=false` |
